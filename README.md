...
| `PNL_POLLING_MODE` | No | bidirectional | Net P&L quote side to poll: `bidirectional` alternates and routes each side to its matching trigger; `buy` or `sell` polls only that side and lets both buy and sell triggers use that observation |
...
| `update-all` | Update operations and bots, optionally switching all to `--branch`; `--leave-stopped` preserves downtime | Git/processes/state marker |
...
Maintenance transfers floor gas against the latest and pending block base fees. A specific
pre-broadcast `max fee per gas less than block base fee` rejection rebuilds and
retries once using the rejection's reported base fee plus a surge margin, a
pending nonce, and fresh gas. Native calculated transfers
